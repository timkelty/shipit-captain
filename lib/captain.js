...
Captain.prototype.envPrompt = function() {
  if (!this.options.targetEnv && this.options.availableEnvs.length > 1) {
    return inquirer.prompt([{
      type: 'list',
      name: 'targetEnv',
      default: this.options.targetEnv,
      message: 'Target environment:',
      choices: this.options.availableEnvs
    }].then(function(answers) {
      this.options.targetEnv = answers.targetEnv;

      return this.options.targetEnv;
    }));
  }

  this.options.targetEnv = this.options.targetEnv || this.options.availableEnvs[0];

  return Promise.resolve(this.options.targetEnv);
}
